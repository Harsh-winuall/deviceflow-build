...
import { gsuiteUsersAdd } from "@/server/settingActions";
...
export const GsuiteDialog = ({
  ...
  // useEffect(() => {
  //   if (gsuiteDialogOpen === "true" && !open) {
  //     setOpen(true);
  //   }
  // }, [gsuiteDialogOpen, open, setOpen]);

  // Clean up URL when dialog closes
  const handleDialogClose = (isOpen: boolean) => {
    setOpen(isOpen);
    if (!isOpen && gsuiteDialogOpen) {
      // Remove the dialog parameter from URL when closing
      const newParams = new URLSearchParams(params);
      newParams.delete("gsuiteDialog");
      newParams.delete("integrationId");
  ...
  const getGsuiteResponse = async () => {
    ...
    console.log("Gsuite Response :", result);

    if (result.data.message === "Seats Over the Limit") {
      setExtraSeatData(result.data);
  ...
  const handleSubmit = async () => {
    if (validateFields()) {
      ...
      currentUrl.searchParams.set("gsuiteDialog", "true");
      ...
      window.location.href = await getGSuiteAuthUrl({
        ...
        onboarding: true,
        price: newprice,
        redirectUri: currentUrl.toString(), // Use the current URL as the redirect URI
      });
      return;
    }
  };
    ...
    <Dialog open={open} onOpenChange={handleDialogClose}>
      <DialogContent className="rounded-2xl bg-white shadow-lg max-w-md p-6 text-center">
        ...
        <ExtraSeatConfirmationDialog
          ...
          followingFn={async () => {
            try {
              const res = await gsuiteUsersAdd(extraSeatData?.blockedUsers);
              setOpen(false);
            } catch (error) {
